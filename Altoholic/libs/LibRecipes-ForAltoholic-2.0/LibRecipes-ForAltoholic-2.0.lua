local MAJOR = "LibRecipes-ForAltoholic-2.0"
...
-- Function added for Altoholic:
-- Reverse lookup - pass in the itemID, get the first known spellID that crafts it
function lib:CreatedBy(itemId)
    for spellId, spell in pairs(spells) do
        for recipeId, knownItemId in pairs(spell) do
            if itemId == knownItemId then
                return spellId
            end
        end
    end
